...
#   - Leftover cash is reinvested (default: split by the stocks' initial weights),
#     keeping the same cadence-aware reserve as the single-stock engine.
...
def run_fire_backtest_multi(
    ...
    *,
    ...
    reinvest_target: str = "weights",      # weights|highest_yield|specific
    reinvest_ticker: Optional[str] = None, # target ticker when reinvest_target=="specific"
    ...
    if not specs or initial_amount <= 0:
        return None

    wsum = sum(s.weight for s in specs)
    if wsum <= 0:
        return None
    for s in specs:
        s.weight = s.weight / wsum        # normalise to 1.0
    ...
    # ── common calendar: buy on the first date ALL stocks have data ─────────────
    buy_date = max(s.close.index[0] for s in specs)
    ...
    # Optional reinvest-only asset OUTSIDE the basket: gets NO initial corpus
    # (weight 0 → 0 starting shares) but receives surplus reinvestment, and its
    # own dividends flow into the shared cash pool. Appended only AFTER buy_date/
    # master are fixed from the basket, so a later-listed target never shifts the
    # simulation window; it is reindexed onto the fixed master (leading = 0 price,
    # which the reinvest branch skips via its price>0 guard).
    if reinvest_asset is not None and reinvest_asset.ticker not in {s.ticker for s in specs}:
        rc = reinvest_asset.close.dropna().copy()
        if not rc.empty:
    ...
    for s in specs:
        # fillna(0.0) only matters for a reinvest-only asset that lists AFTER
        # buy_date (leading NaN → 0 price, skipped by the reinvest price>0 guard);
        # basket assets all have data from buy_date so this is a no-op for them.
        px = s.close.reindex(master).ffill().fillna(0.0)
        prices.append(px)
    ...
    # Resolve the "specific" surplus-reinvest target ticker to an index into the
    # (possibly filtered) specs list. If it was dropped/unknown, fall back to weights.
    reinvest_idx = None
    if reinvest_target == "specific" and reinvest_ticker is not None:
        for i, s in enumerate(specs):
            if s.ticker == reinvest_ticker:
                reinvest_idx = i
                break

    for pos, d in enumerate(master):
        d = pd.Timestamp(d)
        price = [float(prices[i].iloc[pos]) for i in range(n)]
        day_div = 0.0
        day_withdrawal = 0.0
        ...
        for i in range(n):
            dps = div_on[i].get(d)
            if dps and shares[i] > 0 and dps > 0:
                gross = shares[i] * dps
                t = gross * tax
                net = gross - t
                tot_gross_div += gross
        ...
        if d in withdraw_dates and not depleted:
                ...
                if sell_priority == "weights":
                    ...
                    for _ in range(n):
                        ...
                        eligible = [i for i in range(n)
                                    if price[i] > 0 and shares[i] * price[i] > 1e-9]
                        if not eligible:
                            break
                    ...
                    order = sorted(range(n), key=lambda i: (_metric(i), i))
                    for i in order:
                        if shortfall <= 1e-9:
                            break
            ...
            if reinvest_surplus and cash > 1e-9 and not depleted:
                ...
                if ni < len(div_dates_sorted):
                    ...
                    investable = cash - reserve
                    if investable > 1e-9:
                        if (reinvest_target == "specific" and reinvest_idx is not None
                                and price[reinvest_idx] > 0):
                            j = reinvest_idx
                            shares[j] += investable / price[j]
                            tot_reinvested_surplus += investable
                            cash -= investable
        ...
        for i in range(n):
            row[asset_cols[i]] = shares[i] * price[i]
            row[f"{asset_cols[i]}__sh"] = shares[i]   # 보유 주식 수량(주)
        timeline_rows.append(row)

    timeline_df = pd.DataFrame(timeline_rows)
    end_date = master[-1]
    end_px = [float(prices[i].iloc[-1]) for i in range(n)]
