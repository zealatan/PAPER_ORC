...
## 0. 목표
**존슨앤드존슨(JNJ) 배당 백테스트 유튜브 영상 1편**을 코카콜라와 **똑같은 프로덕션 스타일**로 제작.
= AI 영상클립(bg/*.mp4) 배경 + enginechart 차트 덱 + 본인 클론 목소리 더빙. (코카콜라 그대로)
산출물: ① 본편(1080p60) ② 썸네일 ③ 쇼츠(세로).

> ⚠ **`blog/animation_style/`(손그림 일러스트)는 별도 실험 폴더 — JNJ와 무관.** JNJ는 이 일러스트 화풍 안 씀.
> 우린 이미 **재사용 엔진·덱·스크립트**를 갖췄으니(§2 재사용), JNJ 특유 작업은 "엔진으로 JNJ 데이터 산출 + 덱 콘텐츠 교체"뿐. (JNJ 백테스트 데이터 자체는 아직 없음 → §3-A에서 생성)
