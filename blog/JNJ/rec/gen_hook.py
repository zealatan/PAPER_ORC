...
"""쇼츠 상단 훅 텍스트 오버레이 PNG 생성 (1080폭, 투명).
   상단: JNJ 로고(빨간 script) → 그 아래 훅 문구 1행 흰색 + 2행 노란 하이라이트."""
import os
...
LOGO = os.path.join(OUT, 'jnj_logo.png')
...
def make(fname, line1, line2, s1=62, s2=100):
    ...
    # JNJ 로고 (상단 중앙, 폭 550). 로고 밑 공백은 조금만 → 아래 문구를 바짝 붙임
...
# 쇼츠1 — 전략(폭락 vs 적립)
make('hook_strat.png', 'JNJ 25년 · 완벽한 폭락 타이밍', '그냥 적립에 졌다')
# 쇼츠2 — 파산(FIRE)
make('hook_fire.png', 'JNJ 배당으로 은퇴', '얼마 쓰면 파산할까?')
