# -*- coding: utf-8 -*-
"""A단계 백테스트 → spec/JNJ.json 되채움.
   2부(파산 시나리오): FIRE 그리드 원금[200k,400k,600k] × 월인출[1k,2k,3k] × 전략[nominal,real].
   gen_fire_corpus.py(코카콜라 골든) 패턴을 JNJ로 이식. 데이터는 엔진, 대본은 인용.
...
ROOT = Path(__file__).resolve().parent.parent          # blog/JNJ
...
close = pd.read_csv(DATA / "jnj_price.csv", parse_dates=["date"]).set_index("date")["close"]
div   = pd.read_csv(DATA / "jnj_div.csv",   parse_dates=["date"]).set_index("date")["div"]
...
print("→ spec/JNJ.json (data + backtest.fire.scenarios) 갱신")
