...
"""D단계 덱 조립 → 자립형 viewable HTML.
   spec(scenarios/strategy/narration/business) → 씬 JSON → jnj_final.html DEFAULT 이후 강제주입.
   철칙: 차트 pts·수치는 backtest 인용. cocacola 패치 IIFE(v8-hook/revert1/survheat1/src1)는
   1889라인 직후 SCENES 덮어쓰기로 무력화.
   사용: python deck/build_deck.py  →  deck/jnj_v1.html · deck/jnj_deck.json
"""
...
def price_trigger_chart():
    ...
    return {"kind": "line", "x": [2000, 2026.6], "y": [ymin, ymax], "yticks": yt, "xticks": XTICKS,
            "series": [{"pts": ps, "c": INK, "w": 2.8, "name": "JNJ 주가", "end": f"${ps[-1][1]}"}],
...
def news_checks():
    return {"title": "JNJ, 요즘 어떤가 — [최신 근황]", "sub": "2026년 기준 · 출처 JNJ IR·SEC",
            "name": "존슨앤드존슨 (JNJ)", "tag": "64년 배당왕",
            "items": [
...
for n in NARR:
    ...
    if tpl == "videohook":
        ...
        if n["sec"] == "hook" and role == 0:      # 후킹 A 파산
            d.update({"_fire": True, "side": "a", "label": "A", "kick": "2000년, JNJ로 은퇴한 두 사람",
            ...
            d.update({"side": "b", "label": "B", "kick": "2000년, JNJ로 은퇴한 두 사람",
        ...
        if chart_id == "reveal":       # 후킹 반전 (명목 A vs B)
            ...
            d.update({"sub": "", "title": "JNJ 주가와 [−30% 폭락] 매수 신호",
...
json.dump(deck, open(ROOT / "deck" / "jnj_deck.json", "w"), ensure_ascii=False, indent=1)

# ── HTML 주입: KEY 교체 + 코카콜라 패치 IIFE 블록 제거 + '저장된 편집 우선' 조건부 주입 ──
#   → HUD 편집(localStorage 저장)이 새로고침 후에도 유지됨. 최초 로드(저장본 없음)에만 하드코딩 주입.
html = (ROOT / "deck" / "jnj_final.html").read_text(encoding="utf-8")
html = html.replace("const KEY = 'tplCatalog_cocacola_v4g';", "const KEY = 'tplCatalog_jnj_v2';")
...
override = ("/* ══ JNJ 덱 주입 — 저장된 편집(localStorage) 있으면 그대로 유지, 없을 때만 주입.\n"
...
(ROOT / "deck" / "jnj_v1.html").write_text(html, encoding="utf-8")
...
print(f"→ deck/jnj_v1.html ({len(html)//1024}KB, 자립형) · deck/jnj_deck.json")
