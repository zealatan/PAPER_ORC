...
DECK = sys.argv[1] if len(sys.argv)>1 else "cocacola_ABswap.json"   # A/B 스왑 반영본
...
SB = [
 ...
 ("2000년, 코카콜라 하나로 은퇴한 두 사람이 있었습니다. 먼저 A. 은퇴자금은 40만 달러, 셋 중에선 적은 편이었죠. 대신 매달 딱 1천 달러씩만 아껴 쓰고, 배당은 한 푼도 안 빼고 전부 재투자했습니다.", "새싹 배경", ["A 인물카드","머니트리","스탯"], ["슬라이드인"], None),
 ("그리고 B. 은퇴자금은 60만 달러, A의 1.5배였죠. 돈이 넉넉하니 매달 3천 달러씩 여유 있게 썼습니다. 이 정도면 평생 걱정 없겠지 했는데 — 2022년, 그의 계좌 잔고는 0. 파산하고 맙니다.", "트레이더 배경", ["B 인물카드","현금다발","스탯"], ["슬라이드인"], None),
 ...
 ("그런데 여기엔 함정이 하나 있습니다. 2000년은 닷컴버블의 정점 — 지금처럼 거의 모든 자산이 비쌌던 시기였죠. 실제로 나스닥은 이후 78%나 무너졌습니다.", "코인 실사", ["전환 타이틀"], ["페이드"], "※ 닷컴 저점: 나스닥 −78% · S&P500 −49%"),
...
# 확장된 dur 반영한 덱 저장
json.dump(d, open("cocacola_final.json","w",encoding="utf-8"), ensure_ascii=False)
