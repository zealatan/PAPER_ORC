...
/* fixed-aspect charts — container width drives height; Plotly (responsive) fills
   it, so charts keep their ratio instead of flattening on wide screens.
   Ratio 16/6.3 = a 16:9 chart with its height cut ~30%. */
div[data-testid="stPlotlyChart"] { aspect-ratio:16/6.3; width:100%; }
div[data-testid="stPlotlyChart"] > div,
div[data-testid="stPlotlyChart"] .js-plotly-plot,
div[data-testid="stPlotlyChart"] .plot-container { height:100% !important; width:100% !important; }
...
with st.container(border=True):
    ...
    # 잉여 배당을 어느 종목에 재투자할지 직접 선택 (여러 개면 초기 비중대로 분산)
    reinvest_target = "select"
    reinvest_pick_ticker = None
    sel_reinv_labels = r3[1].multiselect(
        "잉여 배당 재투자 종목 (시장 전체)", labels, default=sel_labels,
        key=f"reinv_sel_{market}", disabled=not reinvest_surplus,
        help="남는 배당을 재매수할 종목 — 보유 바스켓에 없는 종목도 가능합니다. "
             "보유 안 한 종목은 생활비로 팔지 않고 배당도 무시하는 '성장 적립용'으로 "
             "쌓입니다. 여러 개면 남는 배당을 똑같이 나눠 삽니다. (재투자 OFF면 무시)")
    reinvest_pick_tickers = [tickers[l] for l in sel_reinv_labels]

    ignore_reinvest_dividends = st.toggle(
        "재투자로 산 주식은 배당 제외", value=True, key=f"ignore_reinv_{market}",
        disabled=not reinvest_surplus,
        help="재투자로 추가 매수한 주식에서는 배당이 나오지 않는 것으로 계산 "
             "(배당의 배당 재투자 없음)")
...
meta = []            # display meta: name, ticker, weight%, close, sch, color, reinvest_only
basket_tickers = set()
# reinvest targets that are NOT in the withdrawal basket → load as reinvest-only
extra_reinv_labels = [l for l in sel_reinv_labels if l not in sel_labels]
_n_load = len(sel_labels) + len(extra_reinv_labels)
with st.spinner(f"{_n_load}개 종목 데이터 로딩 중…"):
    for lbl, w in zip(sel_labels, raw_weights):
        ...
        meta.append({"name": nm, "ticker": tk, "weight": float(w),
                     "close": cl, "sch": classify(dv), "reinvest_only": False})

    # reinvest-only holdings (out-of-basket): no initial buy, never sold, no dividend
    for lbl in extra_reinv_labels:
        tk = tickers[lbl]
        nm = lbl.split(" / ")[0]
        price_df = download_price(tk, start, today)
        cl = get_close_series(price_df)
        if cl.empty:
            st.warning(f"재투자 종목 '{tk}' 가격 데이터를 불러오지 못해 제외합니다.")
            continue
        assets.append(AssetSpec(nm, tk, cl, pd.Series(dtype=float), 0.0, reinvest_only=True))
        meta.append({"name": nm, "ticker": tk, "weight": 0.0,
                     "close": cl, "sch": classify(pd.Series(dtype=float)),
                     "reinvest_only": True})

if not any(not m["reinvest_only"] for m in meta):
    st.error("유효한 보유 종목이 없습니다. 다른 종목/기간을 선택하세요.")
    st.stop()

# normalized weights for DISPLAY (basket only; reinvest-only stays 0%)
wtot = sum(m["weight"] for m in meta) or 1.0
for m in meta:
...
def _chip(m):
    tag = ("<b style='color:#fff9ed;'>재투자</b>" if m["reinvest_only"]
           else f'<b style="color:#fff9ed;">{m["wn"]:.0f}%</b>')
    return (
        '<span style="display:inline-flex;align-items:center;gap:.3rem;'
        'background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.1);'
        'border-radius:999px;padding:.15rem .6rem;margin:.15rem .2rem;font-size:.78rem;color:#f2ecd8;">'
        f'<b style="color:{m["color"]};">●</b>{m["name"]} · {m["ticker"]} {tag}</span>')


chips = " ".join(_chip(m) for m in meta)
st.markdown(
...
result = run_fire_backtest_multi(
    ...
    reinvest_target=reinvest_target,   # "select": 사용자가 고른 종목에 재투자
    reinvest_pick_tickers=reinvest_pick_tickers,      # UI: 재투자 종목 다중선택
    ignore_reinvest_dividends=ignore_reinvest_dividends,  # UI: 재투자분 배당 제외
    start_date=start,
)
if result is None:
...
# ── verdict banner ──────────────────────────────────────────────────────────────
basket = " + ".join(m["ticker"] for m in meta if not m["reinvest_only"])
if s["Survived"]:
    st.markdown(
...
def _style(fig, ytitle):
    ...
    fig.update_layout(
        autosize=True, margin=dict(l=14, r=14, t=30, b=12),
...
fig = go.Figure()
for m in meta:
    _nm = f'{m["ticker"]} (재투자)' if m["reinvest_only"] else f'{m["ticker"]} ({m["wn"]:.0f}%)'
    fig.add_trace(go.Scatter(
        x=tl["Date"], y=tl[m["ticker"]], name=_nm,
...
brk = asum.copy()
_ro = brk["Reinvest Only"] if "Reinvest Only" in brk else pd.Series(False, index=brk.index)
brk["배당주기"] = [("재투자 전용" if ro else FREQ_KR.get(sch_by_tk[tk].frequency, sch_by_tk[tk].frequency))
                for tk, ro in zip(brk["Ticker"], _ro)]
brk_show = pd.DataFrame({
    "종목": [f"{nm} · {tk}" + (" 🔁" if ro else "")
             for nm, tk, ro in zip(brk["Name"], brk["Ticker"], _ro)],
    "비중": [("—" if ro else f"{v:.0f}%") for v, ro in zip(brk["Weight %"], _ro)],
    "배당주기": brk["배당주기"],
...
st.markdown(brk_show.to_html(index=False, classes="fire-table", border=0, escape=False),
            unsafe_allow_html=True)
if not reinvest_surplus:
    _reinv_txt = "재투자 안 함"
elif reinvest_pick_tickers:
    _reinv_txt = "재투자 종목 " + ", ".join(reinvest_pick_tickers)
    if len(reinvest_pick_tickers) > 1:
        _reinv_txt += " (똑같이 나눠 재매수)"
    if ignore_reinvest_dividends:
        _reinv_txt += " · 재투자분 배당 제외"
else:
    _reinv_txt = "재투자 종목 미선택 → 전체 비중대로 분산"
st.caption(f"매도 우선순위: {SELL_PRIORITY_DESC[sell_priority]} · 잉여 배당: {_reinv_txt}")

# ── annual breakdown (with per-stock year-end share counts) ─────────────────────
tl2 = tl.copy(); tl2["Year"] = tl2["Date"].dt.year
agg_spec = {"배당세후": ("Dividend", "sum"), "인출": ("Withdrawal", "sum"),
            "연말자산": ("Portfolio Value", "last")}
for m in meta:
    agg_spec[f'sh_{m["ticker"]}'] = (f'SH::{m["ticker"]}', "last")
annual = tl2.groupby("Year").agg(**agg_spec)
annual["매도필요"] = (annual["인출"] - annual["배당세후"]).clip(lower=0)
...
def _qty(row):
    ...
    for m in meta:
        tk = m["ticker"]
        parts.append(f"{tk} {row['sh_' + tk]:,.0f}주")
    return " · ".join(parts)
...
with st.expander("📅 연도별 상세", expanded=False):
    ...
    out = pd.DataFrame({
        "연도": a["Year"].astype(str),
        "배당(세후)": a["배당세후"].map(money),
        "인출": a["인출"].map(money),
        "매도필요": a["매도필요"].map(money),
        "연말 보유수량": a.apply(_qty, axis=1),
        "연말 자산": a["연말자산"].map(money),
    })
    st.markdown(out.to_html(index=False, classes="fire-table", border=0, escape=False),
                unsafe_allow_html=True)

# ── per-stock price charts (one chart each, actual price, 2-column grid) ─────────
st.markdown('<div class="chart-h">📉 종목별 주가</div>', unsafe_allow_html=True)
start_ts = pd.Timestamp(tl["Date"].iloc[0])
pcols = st.columns(2)
for idx, m in enumerate(meta):
    col = pcols[idx % 2]
    tag = " 🔁 재투자" if m["reinvest_only"] else ""
    col.markdown(
        f'<div class="chart-h" style="font-size:.92rem;margin-top:.4rem;">'
        f'{m["ticker"]} · {m["name"]}{tag}</div>', unsafe_allow_html=True)
    cl = m["close"]
    ...
    pf.add_trace(go.Scatter(
        x=cl.index, y=cl.values, name=m["ticker"],
        line=dict(color=m["color"], width=1.8),
        fill="tozeroy", fillcolor=_rgba(m["color"], .08)))
    if not s["Survived"] and s["Depletion Date"] is not None:
        pf.add_vline(x=pd.Timestamp(s["Depletion Date"]), line=dict(color="#e23636", width=1.2),
                     annotation_text="고갈", annotation_position="top", annotation_font_color="#c0392b")
    col.plotly_chart(_style(pf, f"주가 ({sym})"), width="stretch",
                     config={"responsive": True, "displayModeBar": False})
