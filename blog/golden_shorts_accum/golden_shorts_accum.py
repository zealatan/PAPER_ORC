...
HERE=os.path.dirname(os.path.abspath(__file__))   # blog/golden_shorts_accum
...
function reelAnim(root){
  ...
  function drawYAxis(){   /* y축도 flexible: 매 프레임 현재 YMAX 기준 재생성 */
    ya.innerHTML='';
  ...
  function draw(prog){
    ...
    drawXAxis(R,prog);
    /* 현재 보이는 최대값으로 YMAX 동적 설정 → y축 재스케일 */
    var vmax=0;
    ELS.forEach(function(e){var rr=clipX(e.l.pts,R);rr.arr.forEach(function(q){if(q[1]>vmax)vmax=q[1];});if(rr.ey>vmax)vmax=rr.ey;});
    YMAX=Math.max(vmax*1.15,1);
    drawYAxis();
    var _hlhide=false,_labels=[];
    ELS.forEach(function(e){
      ...
      if(e.l.nolabel){e.dot.style.display='none';e.tt.textContent='';e.tv.textContent='';return;}  /* 원금 점선: 점·라벨 없음 */
      ...
      _labels.push({tv:e.tv,ey:ey});
      if(HLY!=null&&Math.abs(ey-HLY)<42&&lx>560&&lx<840) _hlhide=true;  /* 선 라벨과 겹치면 은퇴원금 라벨을 숨김 */
    });
    /* 끝점 값 라벨 세로 겹침 회피: 가까우면 아래로 밀어냄 */
    _labels.sort(function(a,b){return a.ey-b.ey;});
    var _MG=26;
    for(var _i=1;_i<_labels.length;_i++){
      if(_labels[_i].ey-_labels[_i-1].ey<_MG){_labels[_i].ey=_labels[_i-1].ey+_MG;_labels[_i].tv.setAttribute('y',_labels[_i].ey+6);}
    }
...
<div class="legout"><span class="lg"><span class="sw" style="background:#2b6cb0"></span>매달 적립식</span><span class="lg"><span class="sw" style="background:#c2255c"></span>폭락 시 매수</span></div>
