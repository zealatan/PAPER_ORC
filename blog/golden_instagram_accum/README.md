...
- **retire_stocks**(하락장 방어력): 같은 시점 고점 은퇴로 **여러 종목** 비교(`RS_STOCKS=QQQ,SCHD,QYLD` 또는 `TQQQ,QLD`). 고점→저점 구간만(`RS_END`) 하락 3선 + 은퇴원금선. 표=구간최저·낙폭(▲방어). QQQ -40%/SCHD -20%/QYLD -29%, 레버리지 TQQQ -84%/QLD -67%. `META`로 종목 색·유형, 2~3종목 레이아웃 자동.
- 공통: x축 고정+데이터만 리빌, 제목·범례값 흰색(브랜딩 취소), 그래프 애니 2배속, 2페이지 표 글자 확대. 데이터선은 브랜드골드(#fbbf24) 제외.
