...
- **헤드라인**(폰트 62px = QQQ 크기): 1줄 `<종목>`(마젠타 `#d12e77`) + `으로 은퇴`(흰색) · 2줄 `얼마 있어야 할까?`(**옅은 청록 `#abe0d3`** = QQQ 실측). **QQQ 썸네일과 색·크기·위치 통일.**
- 규격: 카드 `SCALE=0.70` · **문구 세로중심 `TEXT_CY=1138`**(QQQ 로고885/문구1138 실측 기준) · 카드-문구 간격 `GAP=44`. 카드는 문구 바로 위(QQQ 로고 위치 근접). 미니그래프 `fmtc`($10M↑ 축약·통화별)·`YEARS=floor(경과연수)`·파산선 $0 라벨 생략.
...
- 2026-07-30(썸네일 규격화): **신용카드 스타일 썸네일 생성기 `gen_thumb.py`** 추가(검정+흰카드70%+헤드라인·종목명 마젠타·결과 미니그래프). SPY 영상 완성(썸네일 이 규격).
- 2026-07-30(썸네일 QQQ 정렬): 헤드라인 **폰트 62px·2줄 옅은 청록 `#abe0d3`·세로위치 `TEXT_CY=1138`** 을 기존 QQQ 썸네일(실측)에 맞춰 통일. (QQQ 자체 썸네일은 중앙 로고 스타일 원본 보존.) **현재 정본(freeze).**
