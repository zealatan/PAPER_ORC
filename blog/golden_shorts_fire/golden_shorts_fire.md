...
> **상태: 동결 템플릿(FROZEN) · 2026-07-29.** 이 문서 + `golden_shorts_fire.py` + `gen_fire_table.py` +
...
> 레퍼런스 빌드: **QQQ**(닷컴 꼭지 2000년 은퇴). 산출물 `golden_shorts_fire_<STOCK>.mp4`(1080×1920, 30fps, **3페이지·약 49초**).
...
- 끝점 라벨 세로 겹침 회피(가까우면 위로 계단식).

## 3. 레이아웃 (2026-07-29 확정)

- **그래프 카드 = 정사각(1:1)**, viewBox `0 0 960 960`(H=960, MT=132, MB=74, ML=64, MR=176), 화면 중앙.
...
- 2026-07-29: **누적 고스트로 대개편**(원금별 5장 → 연속 1장 + 테이블). 정사각 그래프·상단 헤더 제목·범례 인그래프·은퇴원금 강조·인트로 훅. 현재 정본.
