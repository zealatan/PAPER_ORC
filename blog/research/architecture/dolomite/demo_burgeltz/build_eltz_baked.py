...
SHOTS = [
 (0,'hook','real','cut',4,1,'지도→드론 다이브 줌', "독일 서부, 모젤 강 근처 깊은 숲속입니다.",
   "DYNAMIC opening establisher: start on a clean minimal stylized map of Germany (soft grey landmass, no text labels of any kind) as a single glowing cyan location pin drops onto the western Moselle/Eifel region; then a fast Google-Earth-style DIVE ZOOM plunges straight down out of the map through a thin cloud layer and resolves into a real cinematic drone view swooping fast over deep green forested hills and a winding river toward a hidden valley. no words anywhere.", "", ""),
 ...
 (3,'subject','real','cut',5,0,'', "엘츠 성. 세 가문이 한 성에 함께 산, 중세의 공동주택입니다.",
   "a majestic hero shot of Burg Eltz, a slow full 360-degree orbit sweeping all the way around the castle ramparts and the eight towers on their rock; low golden-hour sun rakes across the grey slate roofs and stone walls with warm rim-light, long shadows and glowing highlights on the towers, dramatic clouded sky and soft god-rays streaming through the surrounding forest, the river gleaming far below.",
   "a slow cinematic full 360-degree orbit right around the castle walls, majestic and unhurried, lit by beautiful low golden-hour light", ""),
 (4,'subject','real','cut',4,0,'', "왜 한 지붕 아래 세 집이 모여 살았을까요?",
   ...
   "an accelerating overhead push-down (ZOOM IN) plunging straight toward the packed rooftops as a hard transition into the next shot", "thin cyan brackets grouping three clustered roof blocks and a callout 3 (no words)"),
 (5,'cause','diorama','cut',5,0,'', "1268년, 삼형제가 성을 물려받아 셋으로 나눕니다. 세 집안, 각자 살 곳이 필요했죠.",
   "matching the push-down from the rooftops, the packed roofs resolve into a floating diorama of the single rock-top castle, then a glowing cyan line splits its plan into three coloured ownership zones — one castle divided among three families who each need their own home.",
   "a match-cut punch-in that resolves the rooftops into the floating diorama, then a slow orbit as the plan splits into three zones", "a bold 1268 marker, three coloured zones split by a cyan line, and a callout 3 (numbers only)"),
 (6,'scale','real','cut',4,0,'', "그런데 성터는, 세 면이 강에 감긴 70미터 바위 하나뿐이었습니다.",
   ...
   "a dramatic orbit around the spur, then a fast vertical drop plunging (ZOOM) straight down the 70 m cliff face to the river below", "a red vertical dimension line on the rock counting up to 70 m, cyan arrows on the river with a callout 3 (numbers only)"),
 ...
 (9,'fail','diorama','cut',4,0,'', "그럼 성을 옆으로 넓히면? 바위 끝이라 넓힐 땅이 없었습니다.",
   ...
   "a hard punch-in (ZOOM IN) onto the horizontal dimension bracket as it pinches shut against the sheer cliff edges", "red X marks off both cliff edges and a horizontal dimension bracket pinched to its limit (no text)"),
 ...
 (115,'solution','diorama','cut',5,0,'', "금사자·은사자·물소뿔, 세 집안이 한 성에 층층이 포개졌습니다.",
   "a clean dramatic vertical cross-section slicing straight down through the entire castle from the tower tops to the rock, a dollhouse-style cutaway that reveals the interior divided into three colour-coded family sections — gold, silver and buff-brown — stacked and interlocking floor by floor all the way up the eight towers, crisp engineering-drawing clarity.",
   "a slow lateral truck across the sliced cross-section face with gentle parallax over the three stacked colour-coded sections", "three colour-coded sections bracketed by cyan leader lines and a callout 3 (numbers only)"),
 (12,'scale','diorama','cut',4,0,'', "방 100칸을 좁은 바위에 욱여넣었습니다. 그것도 많은 방에 난방까지 되게.",
   "the camera flies fast toward one tower and PUSHES THROUGH the stone wall into a cutaway honeycomb of about one hundred tiny rooms crammed inside, thin smoke rising from many chimneys on every roof.",
   "a fast push that punches through the tower wall (object transition) then glides across the honeycomb of a hundred rooms", "a big callout counting up to 100 and small heat/chimney markers on many rooms (numbers only)"),
 (13,'solution','real','cont',4,0,'', "안뜰과 우물은 함께 썼습니다. 좁을수록, 공유가 답이었죠.",
   ...
   "a crane-down descending from the high rooftops into the tight shared inner courtyard, settling on the single well", "cyan arrows pointing to one shared courtyard and well (no text)"),
 (14,'cause','real','cut',4,0,'', "세 면을 감은 강과 절벽은, 그대로 성벽이 됐습니다.",
   ...
   "a low orbit following the river around the cliff base up to the towers, then a fast WHIP-PAN up across the valley toward the opposing hilltop as a transition into the next shot", "cyan arrows tracing the river on three sides and a callout 3 (no text)"),
 (15,'fail','real','cut',5,1,'투석 공성 → 온존', "1331년, 대주교가 맞은편에 공성성을 짓고 돌을 퍼부었지만, 끝내 무력으로는 뚫지 못했습니다.",
   "Continuing straight from an incoming whip-pan onto the opposing forested hilltop: DYNAMIC medieval siege — a small stone siege-castle, trebuchets hurling heavy round stone balls that arc across the valley and SMASH against Burg Eltz's cliff and towers with dust and impacts; then a hard PUSH-IN settles to reveal the castle standing completely intact and defiant, spent stone balls scattered harmlessly among the rocks below. a small bold 1331 marker.",
   "", ""),
 (16,'scale','diorama','cut',5,0,'', "그래서 엘츠 성은 861년, 34대 동안 한 번도 무너지지 않았죠.",
   ...
   "a fast dolly gliding along the long timeline dimension line as the counter rapidly ticks up the centuries", "a long dimension line counting up to 861 yr, a callout 34, and a small 1157 start marker (numbers only)"),
