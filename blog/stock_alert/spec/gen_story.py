...
spec["story"]["hook"] = {
  ...
  "question": "같은 40만 달러·같은 STOCK_ALERT·같은 26년, 매달 쓰는 돈만 달랐다.",
}
from collections import OrderedDict
# 씬 삽입/재배치로 번호가 밀리므로 리스트 순서대로 자동 재번호
for i, n in enumerate(narration):
    n["scene"] = i
# ── 사용자 확정 자막 오버라이드 (브라우저 HUD 편집 → STOCK_ALERT_저장_*.json에서 추출) ──
...
md = ["# STOCK_ALERT 대본 (개정판) — 반전 앞으로 + 뉴스 + 물가반영 + survheat/divbars 복원\n"]
cur = None
for n in narration:
    if n["sec"] != cur:
        cur = n["sec"]
...
print("→ spec/STOCK_ALERT.json(story) · narration_draft.json · SCRIPT.md")
