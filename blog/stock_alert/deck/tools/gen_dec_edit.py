#!/usr/bin/env python3
"""dec_pipeline.html(STOCK_ALERT 24씬 지도) + dec_edit_page{N}.html 24개(페이지별 편집 임베드) 생성.
   흐름: dec_pipeline.html →(페이지 클릭)→ dec_edit_page{N}.html →(iframe)→ stock_alert_v1.html?page=N&edit=1
   출력 위치: blog/architecture_docs/ (dec_pipeline.html 옆). iframe은 ../STOCK_ALERT/deck/stock_alert_v1.html 상대참조."""
from pathlib import Path
import html as _h, json

# ── 종목 자동 인식 (이 파일 위치 = <blog>/<STOCK>/deck/tools/gen_dec_edit.py) ──
#    폴더 복사(stock_alert→pg)만 하면 STOCK·경로가 자동으로 새 종목에 맞춰짐.
SELF = Path(__file__).resolve()
STOCK_DIR = SELF.parent.parent.parent            # <blog>/<STOCK>
STOCK = STOCK_DIR.name                            # STOCK_ALERT / PG / ...
PFX = STOCK.lower()                               # 덱 파일 접두사: stock_alert / pg
...
SCENES = [
    ...
    ("enginechart", "STOCK_ALERT 주가 · −30% 폭락 신호",         "tg", "그래프"),
...
mast = (NAV + '<div class="wrap"><header class="mast">'
        '<div class="eyebrow">배투실 · 영상 파이프라인 · <b>3 덱 생성 (STOCK_ALERT)</b></div>'
        '<h1>덱 페이지 편집 지도 — STOCK_ALERT</h1>'
        '<p class="sub"><code>stock_alert_final.html</code> 엔진 + <code>spec</code>·<code>story</code>로 채운 씬 구성. '
        ...
        'dec_pipeline → dec_edit_page{N} → stock_alert_v1.html?page=N&amp;edit=1 · 변경은 localStorage 저장</div></div>')
...
$('#apply').onclick=()=>{
  if(!confirm('현재 구성을 실제 덱에 반영할까요?\n재정렬·추가·삭제가 stock_alert_v1.html에 적용되고 편집 페이지가 재생성됩니다.')) return;
...
pipe = ('<!doctype html><html lang="ko"><head><meta charset="utf-8">'
        ...
        '<title>덱 페이지 편집 지도 — STOCK_ALERT · 배투실</title><style>' + CSS + EDITOR_CSS + '</style></head><body>'
