#!/usr/bin/env python3
"""stock_alert_final.html + stock_alert_deck_dubbed.json(subTimes/subHold 반영) → deck/stock_alert_dubbed.html.
   build_deck.py와 동일한 주입(KEY 교체·패치 IIFE 제거·bg data URI)에 더빙 씬을 넣는다.
   KEY='tplCatalog_stock_alert_dubbed' → 프리뷰/렌더용 자립형(편집 localStorage와 분리)."""
import json, base64
from pathlib import Path
ROOT = Path(__file__).resolve().parent.parent
scenes = json.load(open(ROOT / "deck" / "stock_alert_deck_dubbed.json", encoding="utf-8"))["scenes"]
...
html = html.replace("const KEY = 'tplCatalog_cocacola_v4g';", "const KEY = 'tplCatalog_stock_alert_dubbed';")
blk_start = html.index("/* FIRE 세트")
p = html.index("var VER='src1';", blk_start)
blk_end = html.index("})();", p) + len("})();")
override = ("/* ══ STOCK_ALERT 더빙 덱 주입(subTimes/subHold 포함) ══ */\n"
...
out = ROOT / "deck" / "stock_alert_dubbed.html"
out.write_text(html, encoding="utf-8")
sub_scenes = sum(1 for s in scenes if s.get("subHold"))
print(f"→ deck/stock_alert_dubbed.html ({len(html)//1024}KB) · 씬 {len(scenes)} · subHold 반영 {sub_scenes}")
