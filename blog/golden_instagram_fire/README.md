...
- 헤더: **"○○로 은퇴했다면 / 얼마나 버텼을까?" 2줄 흰색 타이틀**, 가로 중앙. 종목명은 **티커만**(괄호 설명 없음, 한글 회사명은 그대로). 로고 없음.
- 그래프 박스 `(164, 660, 964, 1360)` — accum과 동일(제목여백·크기·범례여백 통일). 검정 배경(패널 없음). 작은 원금은 완성 후 회색 고스트로 정지, 큰 원금만 애니메이션.
- **축**: 은은한 회색 라인(`#242424`)·밝은 회색 글자(`#b3b3b3`)로 구분. 굵은 축선 없음. x축 도메인 확장 리빌.
- **y축은 활성 리빌에 맞춰 확장**(ALT, 기본 ON), 완성 고스트의 축 초과분은 화면 밖으로.
- **데이터선 색: teal `#2dd4bf`(월 소액 인출) + orange `#ff8c42`(월 고액 인출)**. 끝점 **속 빈 링** 마커.
- **은퇴 원금선 = 흰색 가로선**, y축 왼쪽에 "은퇴 원금 / 금액" 2줄 흰 라벨(원금선 높이, 줄간격 최소).
- 값은 **하단 범례**(스와치+인출라벨+금액). 리빌 중 실시간, 완성 시 "생존/파산" 문구. (온차트 박스 없음 — accum 스타일)
- 테스트 기간 `YYYY.MM~YYYY.MM`은 그래프 **좌상단**.
- **플롯 해상도: 일별**(`FIRE_SAMPLE=D` 기본, D/W/M). 인출 주기·최종 결과는 **월별 그대로**, 선 촘촘함만 바뀜.
- 통화 자동 감지(payload `krw`) → 억/만원 vs $K/$M. 데이터가 최신가까지 반영. 스탬프(생존/파산) 기본 끔.

## 재현

```bash
# 1) 데이터(일별 백테스트 JSON). SHORTS_STOCK: QQQ SPY SCHD QYLD JEPI JEPQ HYNIX SEC KTNG SFIRE KT TQQQ ...
#    FIRE_SAMPLE=D(기본)|W|M 로 플롯 해상도 조절(인출은 항상 월별).
...
- **JEPQ** (2022-05 상장) 분할 없음 · 급변 없음(월배당 인컴형)
