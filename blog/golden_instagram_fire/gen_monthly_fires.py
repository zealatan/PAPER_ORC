#!/usr/bin/env python3
"""
gen_monthly_fires.py — 인스타 전용 월별 백테스트 JSON 생성기.
...
CMAP_USD = {2000: "#2b6cb0", 4000: "#c2255c"}
CMAP_KRW = {2_000_000: "#2b6cb0", 4_000_000: "#c2255c"}
...
if STOCK == "PG":
    ...
    close, div = yf_series("030200.KS"); cpi = kr_cpi(); KRW = True; TAX = 15.4
else:
...
if KRW:
    PRIN = [2, 4, 6, 8, 10]; PRIN = [p * 100_000_000 for p in PRIN]; MOS = [2_000_000, 4_000_000]; CMAP = CMAP_KRW
    def money(v): return ("%.1f억" % (v / 1e8)) if v >= 1e8 else ("%d만" % round(v / 1e4))
    def hnum(v): return money(v).replace(".0", "")
else:
    PRIN = [200_000, 400_000, 600_000, 800_000, 1_000_000]; MOS = [2000, 4000]; CMAP = CMAP_USD
