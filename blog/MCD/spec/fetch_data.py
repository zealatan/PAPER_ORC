# -*- coding: utf-8 -*-
"""MCD 원시 데이터 조달 → 재현용 CSV 저장 (코카콜라 골든과 동일 형식).
   사용: python spec/fetch_data.py
   산출: data/mcd_price.csv (date,close 무수정 종가) · data/mcd_div.csv (date,div 주당배당)
"""
import csv
...
with open(OUT / "mcd_price.csv", "w", newline="") as f:
    w = csv.writer(f); w.writerow(["date", "close"])
    for d, c in prices:
        w.writerow([d.isoformat(), f"{c:.6f}"])
with open(OUT / "mcd_div.csv", "w", newline="") as f:
    w = csv.writer(f); w.writerow(["date", "div"])
    for d, v in divs:
        w.writerow([d.isoformat(), f"{v:.6f}"])

print(f"[MCD] 가격 {len(prices)}행 ({prices[0][0]}~{prices[-1][0]}), 배당 {len(divs)}건")
...
print(f"→ {OUT}/mcd_price.csv · mcd_div.csv")
