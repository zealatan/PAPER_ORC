...
"""쇼츠 상단 훅 텍스트 오버레이 PNG 생성 (1080폭, 투명).
   상단: PG 로고(빨간 script) → 그 아래 훅 문구 1행 흰색 + 2행 노란 하이라이트."""
import os
...
LOGO = os.path.join(OUT, 'pg_logo.png')
...
def make(fname, line1, line2, s1=60, s2=96):
    ...
    # PG 아치 로고 — 작게 줄여 '상단 중앙 배지'로. 텍스트와 겹치지 않게 위로 분리
...
make('hook_fire.png', 'PG 주식 40만 달러로', '파이어 가능할까?')
