...
narration = [
 ...
 {"scene": 5, "sec": "intro", "tpl": "herostat", "lines": [
   ...
   f"P&G는 {BIZ['div_years']}년 연속 배당을 늘려온 배당왕 — 전 세계에 단 여섯 곳뿐인 희귀한 기업입니다.",
   "세제·기저귀 같은 생활필수품을 파니, 불황에도 흔들리지 않는 방어주죠."]},
 {"scene": 6, "sec": "intro", "tpl": "kpirow", "lines": [
   "그럼 P&G는 뭘 팔아 이 배당을 지탱할까요?",
   f"타이드 세제, 팸퍼스 기저귀, 질레트 면도기까지 — {BIZ['brands_count']}개 핵심 브랜드를 거느린 생활필수품 제국입니다.",
   f"가장 큰 부문은 섬유·홈케어로, 전체 매출의 {BIZ['segments'][0]['share_pct']}퍼센트를 차지하죠."]},
 {"scene": 7, "sec": "intro", "tpl": "checks", "lines": [
   "요즘 실적은 어떨까요?",
   f"2025 회계연도 매출은 843억 달러로, 유기적으로 {BIZ['organic_sales_pct']}퍼센트 성장했고,",
   f"순이익은 {BIZ['net_income_growth_pct']}퍼센트 늘어난 161억 달러, 주당순이익은 {BIZ['eps_growth_pct']}퍼센트 뛰었습니다.",
...
spec["story"]["hook"] = {
  ...
  "question": "같은 40만 달러·같은 PG·같은 26년, 매달 쓰는 돈만 달랐다.",
}
from collections import OrderedDict
# 씬 삽입/재배치로 번호가 밀리므로 리스트 순서대로 자동 재번호
for i, n in enumerate(narration):
    n["scene"] = i
# ── 사용자 확정 자막 오버라이드 (브라우저 HUD 편집 → PG_저장_*.json에서 추출) ──
...
S.save_spec(spec, str(ROOT / "spec" / "PG.json"))
json.dump(narration, open(ROOT / "spec" / "narration_draft.json", "w"), ensure_ascii=False, indent=1)

md = ["# PG 대본 (개정판) — 반전 앞으로 + 뉴스 + 물가반영 + survheat/divbars 복원\n"]
cur = None
for n in narration:
    if n["sec"] != cur:
        cur = n["sec"]
...
print("→ spec/PG.json(story) · narration_draft.json · SCRIPT.md")
