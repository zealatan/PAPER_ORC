#!/usr/bin/env python3
"""pg_final.html + pg_deck_dubbed.json(subTimes/subHold 반영) → deck/pg_dubbed.html.
   build_deck.py와 동일한 주입(KEY 교체·패치 IIFE 제거·bg data URI)에 더빙 씬을 넣는다.
   KEY='tplCatalog_pg_dubbed' → 프리뷰/렌더용 자립형(편집 localStorage와 분리)."""
import json, base64
from pathlib import Path
ROOT = Path(__file__).resolve().parent.parent
scenes = json.load(open(ROOT / "deck" / "pg_deck_dubbed.json", encoding="utf-8"))["scenes"]
...
html = html.replace("const KEY = 'tplCatalog_cocacola_v4g';", "const KEY = 'tplCatalog_pg_dubbed';")
blk_start = html.index("/* FIRE 세트")
p = html.index("var VER='src1';", blk_start)
blk_end = html.index("})();", p) + len("})();")
override = ("/* ══ PG 더빙 덱 주입(subTimes/subHold 포함) ══ */\n"
...
out = ROOT / "deck" / "pg_dubbed.html"
out.write_text(html, encoding="utf-8")
sub_scenes = sum(1 for s in scenes if s.get("subHold"))
print(f"→ deck/pg_dubbed.html ({len(html)//1024}KB) · 씬 {len(scenes)} · subHold 반영 {sub_scenes}")
