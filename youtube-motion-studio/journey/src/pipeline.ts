...
import { SCENARIOS, type ScenarioDef } from "./projects/index";

const HERE = dirname(fileURLToPath(import.meta.url));
export const SCENARIOS_DIR = join(HERE, "..");
const FONTS_DIR = join(SCENARIOS_DIR, "fonts");
...
const dir = (id: string): string => join(SCENARIOS_DIR, id);
...
export function loadProject(id: string): MotionProject {
  ...
  return SCENARIOS.find((e) => e.id === id)!.build();
}
...
export function generate(ex: ScenarioDef): void {
...
export function preview(ex: ScenarioDef): void {
  ...
  writeFileSync(
    ...
    renderFrameToPng(resolved, heroT, registry, 1080, false, FONTS),
  );
...
export async function render(ex: ScenarioDef): Promise<{ renderTimeMs: number }> {
  const project = loadProject(ex.id);
  ensure(dir(ex.id));
...
export function validate(ex: ScenarioDef, timing: { renderTimeMs: number }): Validation {
  const project = loadProject(ex.id);
