...
import { SHOWCASES } from "./projects/index";
import { SHOWCASE_DIR, type Validation } from "./pipeline";

function load(): Validation[] {
  const out: Validation[] = [];
  for (const s of SHOWCASES) {
    const p = join(SHOWCASE_DIR, s.id, "validation.json");
...
export function buildGallery(): { count: number } {
  const vals = load();
  writeFileSync(
    join(SHOWCASE_DIR, "gallery", "report.json"),
    JSON.stringify(
      {
        total: vals.length,
  ...
  writeFileSync(join(SHOWCASE_DIR, "gallery", "index.html"), html(vals));
...
<title>YouTube Motion Studio — Showcase</title>
<style>
...
<body>
  <header><h1>YouTube Motion Studio — Cinematic Showcase</h1>
